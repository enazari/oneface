...
def return_generator_model_and_imgsize(latent_dim=512):
    ...
    model.add(Conv2DTranspose(100, kernel_size=8, strides=(2, 2), activation='relu'))
    model.add(BatchNormalization(momentum=0.8))

    model.add(Conv2DTranspose(100, kernel_size=11, strides=(2, 2), activation='relu'))
    model.add(BatchNormalization(momentum=0.8))

    model.add(Conv2DTranspose(100, kernel_size=11, strides=(2, 2), activation='relu'))
    model.add(BatchNormalization(momentum=0.8))

    model.add(Conv2DTranspose(3, kernel_size=12, strides=(2, 2), activation='linear'))
    model.add(BatchNormalization(momentum=0.8))
